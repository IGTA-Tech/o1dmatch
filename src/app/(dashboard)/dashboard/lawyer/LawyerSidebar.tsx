...
// 1. Added 'Affiliate Program' nav item under Partner section (only when isPartner)
// 2. Fixed partner banner copy: 15% commission (was 20%)
// All other layout, styling, and mobile sidebar logic UNCHANGED.
// ────────────────────────────────────────────────────────────
...
import {
  LayoutDashboard, User, Users, BarChart3, Mail,
  Menu, Bell, HelpCircle, Star, Layers, TrendingUp,
} from 'lucide-react';
...
// Base nav sections — always shown
...
// Partner-only section — shown only when isPartner
const PARTNER_NAV_SECTION = {
  label: 'Partner',
  items: [
    { href: '/dashboard/lawyer/affiliate', label: 'Affiliate Program', icon: TrendingUp },
  ],
};

export default function LawyerSidebar({
  lawyerName, lawyerInitials, isPartner, children,
}: LawyerSidebarProps) {
  ...
  const sidebarContent = (
    <>
      ...
      {/* Partner badge — UNCHANGED layout, fixed copy */}
      {isPartner && (
        <div style={{
      ...
      <div className="px-6 py-4 border-t border-white/[0.06]">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <div className="text-sm font-semibold text-white truncate">{lawyerName}</div>
