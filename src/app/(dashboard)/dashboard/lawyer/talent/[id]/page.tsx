// src/app/(dashboard)/dashboard/lawyer/talent/[id]/page.tsx
...
export default async function LawyerTalentDetailPage({
  params,
}: {
  ...
  const { data: lawyerProfile } = await supabase
    ...
    .eq('user_id', user.id)
    .single();
  if (!lawyerProfile) redirect('/dashboard/lawyer');
  ...
  if (error) console.error('[lawyer/talent/[id]]', error);
  if (!talent) redirect('/dashboard/lawyer');
  ...
  // Connection requests from this talent to THIS lawyer
  const { data: connectionRequests } = await supabase
    .from('lawyer_connection_requests')
    .select('id, status, message, created_at, requester_type')
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* Connection Requests from this talent */}
            {connectionRequests && connectionRequests.length > 0 && (
              <Section title="Connection Requests" icon={<Users className="w-4 h-4" />}>
