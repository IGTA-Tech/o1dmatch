// src/app/(dashboard)/dashboard/lawyer/affiliate/page.tsx
...
export default async function LawyerAffiliatePage() {
  ...
  const { data: lawyerProfile } = await supabase
    ...
    .select('is_partner, attorney_name')
    .eq('user_id', user.id)
    .single();

  if (!lawyerProfile?.is_partner) redirect('/dashboard/lawyer');
  ...
  if (!partner) redirect('/dashboard/lawyer');
  ...
  // Active subscriptions = pending or approved commissions
  ...
  return (
    <AffiliateDashboard
      ...
      payouts={payouts ?? []}
      userName={lawyerProfile.attorney_name ?? 'Attorney'}
