// src/app/api/send-interest-letter-confirmation/route.ts
//
// Sends a confirmation email to the employer after they successfully
// send an Interest Letter to a talent.
//
// All data is passed directly from the client (no sensitive info — no
// talent name/email is included in the email body per project rules).
// Uses SendGrid HTTP API (HTTPS) — works on Vercel.
...
function buildConfirmationHtml({
  ...
  const detailRow = (label: string, value: string) => `
    <tr>
      ...
      <td style="padding:10px 0 10px 16px;border-bottom:1px solid #F1F5F9;
                 vertical-align:top;">
          ...
          <tr>
            <td style="background:#0B1D35;border-radius:16px 16px 0 0;
              ...
              <p style="margin:12px 0 0;font-size:14px;
                ...
                Your letter has been delivered to the candidate.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background:#FFFFFF;padding:36px 40px;">
              ...
              <p style="margin:0 0 28px;font-size:15px;color:#374151;line-height:1.7;">
                Your Interest Letter for the
                <strong style="color:#0B1D35;">${jobTitle}</strong>
                position has been successfully sent on behalf of
                <strong style="color:#0B1D35;">${companyName}</strong>.
                Here is a summary of the letter details:
              </p>
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.error('[send-interest-letter-confirmation] SENDGRID_API_KEY is not set');
    ...
    await sendEmail(
      apiKey,
      toEmail,
      `Your interest letter for "${jobTitle}" has been sent – O1DMatch`,
    ...
    console.log('[send-interest-letter-confirmation] Email sent to:', toEmail);
    return NextResponse.json({ success: true });

  } catch (err) {
    console.error('[send-interest-letter-confirmation] Unexpected error:', err);
